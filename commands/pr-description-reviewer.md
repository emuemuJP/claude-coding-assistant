...
└─ NO  → 10秒待機後、リトライ（1回目）
        ...
        └─ NO  → 10秒待機後、リトライ（2回目・最終）
                ...
                60秒以内に応答あり？
                ├─ YES → ステップ4.2へ
                └─ NO  → Codexなしでレビュー続行（ステップ5へ）
```

**動作確認結果:**
- ✅ **成功**: ステップ4.2に進んで妥当性検証を実施
- ❌ **失敗（3回試行後も応答なし）**: Codexなしでレビューを続行（ステップ5へ）
...
妥当性検証リクエスト送信
    ↓
90秒以内に応答あり？
├─ YES → フィードバック反映してステップ5へ
└─ NO  → 10秒待機後、リトライ（1回のみ）
            ↓
        90秒以内に応答あり？
        ├─ YES → フィードバック反映してステップ5へ
        └─ NO  → Codex検証なしでステップ5へ（レビュー投稿）
```

**スタック防止チェック:**
- 妥当性検証が2回タイムアウトした場合、Codex検証なしでレビューを投稿
- Codex通信の合計時間が5分を超えた場合、Codex検証をスキップ
...
- **エラーメッセージが返された場合**: Codex検証なしでレビューを投稿
