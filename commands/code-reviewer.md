...
**関連コマンド**: PR自体の記載（タイトル、説明文）をレビューしたい場合は `@pr-description-reviewer.md` を使用してください。
...
**重要**: Codexとの通信でスタックを防ぐため、以下のポリシーを厳守してください。

| 項目 | 設定値 |
|------|--------|
| 動作確認タイムアウト | 60秒 |
| 妥当性検証タイムアウト | 90秒（各コメントごと） |
| リトライ回数 | 最大2回（動作確認）、最大1回（妥当性検証） |
| リトライ間隔 | 10秒 |
| Codex通信合計上限 | 10分（超過時は残りをCodexなしで続行） |
...
└─ NO  → 10秒待機後、リトライ（1回目）
        ...
        60秒以内に応答あり？
        ├─ YES → ステップ5.2へ
        └─ NO  → 10秒待機後、リトライ（2回目・最終）
                    ↓
                60秒以内に応答あり？
                ├─ YES → ステップ5.2へ
                └─ NO  → Codexなしでレビュー続行（ステップ6へ）
```

**動作確認結果:**
- ✅ **成功**: ステップ5.2に進んで妥当性検証を実施
- ❌ **失敗（3回試行後も応答なし）**: Codexなしでレビューを続行（ステップ6へ）
...
各コメントの妥当性検証に対して、以下のタイムアウト処理を適用します：

```
妥当性検証リクエスト送信
    ↓
90秒以内に応答あり？
├─ YES → フィードバック反映して次のコメントへ
└─ NO  → 10秒待機後、リトライ（1回のみ）
            ↓
        90秒以内に応答あり？
        ├─ YES → フィードバック反映して次のコメントへ
        └─ NO  → このコメントはCodex検証なしで投稿（次のコメントへ）
```

**スタック防止チェック:**
- 個別コメントの検証が2回タイムアウトした場合、そのコメントはCodex検証なしで投稿
- Codex通信の合計時間が10分を超えた場合、残りのコメントはすべてCodex検証なしで投稿
- 連続して3つのコメント検証がタイムアウトした場合、以降のCodex検証をスキップ
...
**異常応答への対処**:
- **応答が途中で切れた場合**: 得られた情報のみを活用し、不明点は無視して続行
- **応答形式が異なる場合**: 内容を解釈できる範囲で活用、解釈不能なら検証なしで続行
- **エラーメッセージが返された場合**: そのコメントはCodex検証なしで投稿
